...
def normalize_model_name(model_name: str) -> str:
  """Normalise les noms de modèles pour gérer différentes façons de les écrire"""
  if not model_name:
    return None
    
  # Remplacer les tirets et points par des caractères communs pour la normalisation
  normalized = model_name.lower().replace('_', '-').replace(' ', '-')
  
  # Gérer les variantes spécifiques
  if normalized in ['llama3.2-8b', 'llama3.2-8', 'llama-3.2-8b']:
    return 'llama-3-8b'  # Format standard dans notre système
  if normalized in ['llama3.2-1b', 'llama3.2-1']:
    return 'llama-3.2-1b'
  if normalized in ['llama3.2-3b', 'llama3.2-3', 'llama-3.2-3']:
    return 'llama-3.2-3b'
  if normalized in ['llama3.1-8b', 'llama3.1-8']:
    return 'llama-3.1-8b'
  if normalized in ['llama3.1-70b', 'llama3.1-70']:
    return 'llama-3.1-70b'
    
  # Pour les autres cas, garder le nom tel quel
  return model_name
...
class ChatGPTAPI:
  ...
  async def handle_post_chat_completions(self, request):
    ...
    # Normaliser le nom du modèle pour gérer différentes façons de l'écrire
    if chat_request.model:
      normalized_model = normalize_model_name(chat_request.model)
      if normalized_model != chat_request.model:
        if DEBUG >= 1: print(f"[ChatGPTAPI] Normalized model name from '{chat_request.model}' to '{normalized_model}'")
        chat_request.model = normalized_model
    
    # Gérer les modèles GPT compatibles
